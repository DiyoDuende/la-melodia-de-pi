...
// ============================================================
// 2. CONSTANTES Y SINCRONIZACIÓN DE TIEMPO
// ============================================================
...
// ============================================================
// 4. NOTAS Y ALTURAS (para el pentagrama)
// ============================================================
const NOTAS = {
...
async function cargarTraducciones() {
    try {
        ...
        aplicarTraduccion();
        // Una vez cargadas, regenerar el pentagrama para traducir las notas
        if (!modoVivo) {
...
function cambiarIdioma(idioma, btn) {
    ...
    // Regenerar el pentagrama con el nuevo idioma
    if (modoVivo) {
        if (worker) {
            const segundo = getSegundoGlobal();
            worker.postMessage({ id: 'pentagrama', inicio: segundo - 2, cantidad: 5 });
...
document.addEventListener("DOMContentLoaded", async () => {
    // 1. Pinta el pentagrama de inmediato (sin esperar)
    if (document.getElementById('notasPentagrama')) {
        generarPentagramaInicial();
    }

    // 2. Luego carga las traducciones
    cargarTraducciones();

    // 3. Configurar botones de idioma
    ...
    // 4. Si es página principal, iniciar el resto (sala global, intérpretes)
    if (esPaginaPrincipal) {
        conectarSalaGlobal();
